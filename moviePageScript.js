...
function populateMoviePage(myJson) {
    ...
    // filter out movie videos array to get trailer link
    ...
    // setting youtube url based on filtered results
    var ytURL;
    ...
    // setting up html for whole page
    ...
    `;
    // appending above created child to movie display div
    document.getElementById('movie-display').appendChild(eachMovieDiv);


    // on click of #trailer-div-btn, it will change the display property of mini player to make it visible
    ...
    // on clicking x, it will change display back to none
    document.getElementById('remove-video-player').addEventListener('click', function (){
        // make the div invisible and stop video
        stopThis();
        trailerVideoDiv.style.display = 'none';
    });

    // this function will stop the video
    function stopThis(){
