...
// populating best movies accoring to year dropdown using js
...
// calling function to populate the home page 
...
function createMovieElement(movie){
    // create the movie element and add class and HTML to it
    ...
    `;
    // append this newly created child div to the parent movie-container
    movieContainer.appendChild(movieElement);
}


//each movie element image is an <a> tag which will redirect to movie page with movie name and its rating as query string. This passed data will then be used to fetch individual movie details from OMDB api and display them. rating is passed specifically to avoid mismatch issues
...
function addingMovieToList(buttonID){
    ...
    //first we need to check if local storage is empty, if yes then push data directly; if not, then first retrieve that data, modify it and then append modified data to local storage;
...
// toast will be displayed for 2 seconds
...
// search bar functionality by directly modifing dom elements instead of providing a search list
